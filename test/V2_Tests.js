...
describe("SsacCommunityV2", function () {
  ...
  async function deployV1() {
    ...
    const proxy = await upgrades.deployProxy(SsacCommunityV2, [voteFee]);
  ...
  // 15th Test
  it("should return the ID of the winner", async function () {
    const {
      proxy,
      user1,
      user2,
    ...
    // Owner/Chainlink VRF ends the Contest
    await proxy.endContest(0); // Ending the First Contest (ID 0)
    expect(await proxy.getContestWinner(0)).to.equal(2);
  });
});
